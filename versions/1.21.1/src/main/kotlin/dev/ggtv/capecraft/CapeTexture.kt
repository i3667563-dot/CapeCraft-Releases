...
object CapeTexture {
    ...
    private fun copyPixels(image: NativeImage, w: Int, h: Int, frame: IntArray) {
        ...
        for (i in frame.indices) {
            image.setColor(x, y, argbToAbgr(frame[i]))
            x++
            if (x == w) {
                x = 0
                y++
    ...
    /** ARGB → ABGR (NativeImage.setColor в 1.21.1 хранит пиксели как RGBA little-endian). */
    private fun argbToAbgr(argb: Int): Int =
        ((argb ushr 24) and 0xFF shl 24) or
            ((argb and 0xFF) shl 16) or
            ((argb ushr 8) and 0xFF shl 8) or
