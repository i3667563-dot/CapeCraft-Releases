...
/** Тесты резолвера — порт tests/resolver.rs из Rust-проекта Cren. */
class ResolverTest {
    ...
    @Test
    fun `load from file`() {
        val tmp = java.nio.file.Files.createTempFile("kjen", ".crn")
    ...
    fun `load missing file is io error`() {
        val e = assertFailsWith<CrenError.Io> { CrenConfig.load("/nonexistent/путь/x.crn") }
